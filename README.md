This is a concourse resource for helping to manage certificates using acme.sh.  There is a check script (to check and fetch a new certificate if there is one) and an in script (to fetch the current certificate for use in a concourse pipeline).  It uses a versined S3 bucket to store the certificates, and requires a IAM user which have a key and secret to assume an IAM role to write to the S3 bucket.
...
```
It will issue a certificate if none exists in the s3 bucket in the path `certificates/my.domain_ecc.zip`, copy this certificate to the bucket and emit a version.  This version will remain the same unless there is a change to the domains (eg. and additonal alt domain is added or removed), or the acme 'next renew time is passed', where a new certificate will be issued and a new version returned.
